// ======================================================
// AUTH.JS - LÓGICA DE AUTENTICAÇÃO E REDIRECIONAMENTO
// ======================================================
// Este arquivo é responsável por:
// 1) definir a URL correta do backend;
// 2) verificar se o usuário está logado;
// 3) fazer login e cadastro via fetch;
// 4) redirecionar para páginas corretas.

// Define a base da API dependendo do ambiente atual.
// Em localhost, o frontend pode estar em porta diferente do backend,
// então precisamos apontar para o servidor local da API.
const BASE_URL = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1"
    ? "http://localhost:3000"
    : "https://coffee-center-2.onrender.com";

// Verifica se a página atual está sendo servida localmente.
function paginaNoServidorLocal() {
    const host = window.location.hostname;
    return host === "localhost" || host === "127.0.0.1";
}

// Monta a URL da página correta, levando em conta o ambiente local ou produção.
// Isso evita que uma rota de página dê erro quando o site é acessado de outra origem.
function urlPagina(pagina) {
    if (paginaNoServidorLocal() && !window.location.href.includes("http://localhost:3000")) {
        return `http://localhost:3000/pages/${pagina}`;
    }

    if (window.location.pathname.includes("/pages/")) {
        return `${window.location.origin}/pages/${pagina}`;
    }

    return `${window.location.origin}/${pagina}`;
}

// Redireciona para a página de login.
function irParaLogin() {
    window.location.assign(urlPagina("login.html"));
}

// Redireciona para a página do cardápio.
function irParaCardapio() {
    window.location.assign(urlPagina("cardapio.html"));
}

// Verifica se o usuário está autenticado antes de abrir o cardápio.
// Se não estiver, manda para o login.
async function verificarSessao() {
    const estaNaPaginaCardapio = window.location.pathname.includes("cardapio.html");
    if (!estaNaPaginaCardapio) return true;

    try {
        const resposta = await fetch(`${BASE_URL}/me`, {
            credentials: "include"
        });

        const dados = await resposta.json().catch(() => ({}));

        if (!resposta.ok || !dados.logado) {
            irParaLogin();
            return false;
        }

        return true;
    } catch {
        irParaLogin();
        return false;
    }
}

// Quando entra na página de cardápio, a sessão é checada imediatamente.
if (window.location.pathname.includes("cardapio.html")) {
    verificarSessao();
}

// Busca o formulário de login no HTML.
const formLogin = document.getElementById("formLogin");
if (formLogin) {
    // Escuta o evento de envio do formulário.
    formLogin.addEventListener("submit", async function (e) {
        e.preventDefault();

        // Pega os valores digitados pelos usuários.
        const email = document.getElementById("emailLogin").value;
        const senha = document.getElementById("senhaLogin").value;
        const mensagem = document.getElementById("mensagemLogin");

        try {
            // Envia os dados para a rota /login do backend.
            const resposta = await fetch(`${BASE_URL}/login`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                credentials: "include",
                body: JSON.stringify({ email, senha })
            });

            if (resposta.ok) {
                mensagem.textContent = "Login realizado com sucesso!";
                setTimeout(() => {
                    irParaCardapio();
                }, 300);
            } else {
                const dados = await resposta.json();
                mensagem.textContent = dados.erro || "E-mail ou senha incorretos.";
            }
        } catch {
            mensagem.textContent = "Erro ao conectar com o servidor.";
        }
    });
}

// Busca o formulário de cadastro.
const formCadastro = document.getElementById("formCadastro");
if (formCadastro) {
    formCadastro.addEventListener("submit", async function (e) {
        e.preventDefault();

        // Coleta os dados digitados para criação do usuário.
        const nome = document.getElementById("nome").value;
        const email = document.getElementById("email").value;
        const senha = document.getElementById("senha").value;
        const confirmasenha = document.getElementById("confirmasenha").value;
        const mensagem = document.getElementById("mensagemCadastro");

        // Valida se as senhas digitadas são iguais.
        if (senha !== confirmasenha) {
            mensagem.textContent = "As senhas não coincidem.";
            return;
        }

        try {
            const resposta = await fetch(`${BASE_URL}/cadastro`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                credentials: "include",
                body: JSON.stringify({ nome, email, senha })
            });

            if (resposta.ok) {
                mensagem.textContent = "Cadastro realizado! Redirecionando...";
                setTimeout(() => {
                    irParaLogin();
                }, 1500);
            } else {
                const dados = await resposta.json();
                mensagem.textContent = dados.erro || "Erro ao realizar cadastro.";
            }
        } catch {
            mensagem.textContent = "Erro ao conectar com o servidor.";
        }
    });
}